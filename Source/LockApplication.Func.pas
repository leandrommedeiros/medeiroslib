unit LockApplication.Func;

interface

uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms,
  Dialogs, StdCtrls, ExtCtrls, Registry, LockApplication.Controle,
  SHFolder,inifiles;
...
uses
   LockApplication.Vars, LockApplication.Interfaces, Lib.StrUtils;
...
begin
   if versao_teste then
      begin
          ...
          'eletronica-repa@bol.com.br', mtWarning, [ mbOK],0);
...
begin
  ...
  try
    LocalCR          := '\' + CR(IntToStr(Variaveis.ID_Sistema), Variaveis.Chave_Crypt);
    Registro.RootKey := HKEY_CURRENT_USER;
    Resultado        := Registro.OpenKey(Variaveis.Local_Registro + LocalCR, False);
  except
...
begin
  ...
  begin
    ...
    if posLetra = 0 then posLetra := ord(Result[i]);
...
begin
  ...
  {$ELSE}
  TryStrToInt(Lib.StrUtils.GenRandomHash(), iRandom);
  a := iRandom;
  {$ENDIF}
